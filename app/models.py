...
class User(PaginatedAPIMixin, db.Model, UserMixin):
    ...
    posts = db.relationship('Post', backref='author', lazy='dynamic')  # Настройка один - ко многим
    about_me = db.Column(db.String(140))
    last_seen = db.Column(db.DateTime, default=datetime.utcnow)
    # Подписчики многие - ко многим
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue.enqueue('app.task.' + name, self.id,
                                                *args, **kwargs)
    ...
    def abort_tasks(self):
        tasks = self.get_tasks_in_progress()
        job = get_current_job()
        if job:
            job.cancel()
        for task in tasks:
